...
class ModelTrainer:
    ...
    def prepare_labels_for_xgboost(self, y_train, y_test):
    ...
    def train_model(self, model_name, model, X_train, y_train, X_test, y_test):
        ...
        # Convert labels for XGBoost (requires 0-based labels)
        if model_name == 'xgboost':
            y_train_model = y_train.replace({-1: 0, 1: 1})
            y_test_model = y_test.replace({-1: 0, 1: 1})
        else:
        ...
        try:
            ...
            train_acc = accuracy_score(y_train_model, model.predict(X_train))
            test_acc = accuracy_score(y_test_model, model.predict(X_test))
            ...
            self.training_history[model_name] = {
                ...
                'train_accuracy': train_acc,
                'test_accuracy': test_acc,
                'run_id': self.mlflow_tracker.get_run_id() if self.mlflow_tracker.enabled else None
            }
