...
class ModelEvaluator:
    def __init__(self, config_path="src/config/config.yaml"):
        ...
        self.results = {}
        self.models = {}  # Store models for saving best one
    ...
    def evaluate_model(self, model_name, model, X_test, y_test):
        ...
        try:
            # Convert labels for XGBoost (it predicts 0/1)
            if model_name == 'xgboost':
                y_test_eval = y_test.replace({-1: 0, 1: 1})
            else:
                y_test_eval = y_test
            ...
            metrics = {
                ...
                'precision': precision_score(y_test_eval, y_pred, average='binary', zero_division=0),
                'recall': recall_score(y_test_eval, y_pred, average='binary', zero_division=0),
                'f1_score': f1_score(y_test_eval, y_pred, average='binary', zero_division=0),
                'roc_auc': roc_auc_score(y_test_eval, y_pred_proba) if y_pred_proba is not None else 0
            }
            ...
            class_report = classification_report(y_test_eval, y_pred, output_dict=True)

            # Store results
            self.results[model_name] = {
                'model': model,  # Store model reference
                'metrics': metrics,
    ...
    def evaluate_all_models(self, trained_models, X_test, y_test):
        ...
        self.models = trained_models  # Store models
    ...
    def generate_comparison_table(self):
        ...
        # Only sort if 'accuracy' column exists
    ...
    def save_best_model(self, metric='accuracy'):
        """Save the best performing model for serving"""
        if not self.results:
            logger.warning("No evaluation results to determine best model")
            return None

        # Find best model
        best_model_name, best_score = self.get_best_model(metric)
        ...
        # Save directory
        models_dir = Path(self.config['training']['save_model_path'])
        models_dir.mkdir(parents=True, exist_ok=True)

        # 1. Save as "best_model.pkl" (for serving)
        ...
        metadata = {
            ...
            'all_metrics': self.results[best_model_name]['metrics'],
            'timestamp': datetime.now().isoformat(),
            'confusion_matrix': self.results[best_model_name]['confusion_matrix']
        }
        ...
        logger.info(f"Type: {type(best_model).__name__}")
        logger.info(f"Accuracy: {best_score:.4f}")
    ...
    def run(self, trained_models, X_test, y_test):
        ...
        # Get best model
        best_model_name, best_score = self.get_best_model('accuracy')
        ...
        best_model_path, _ = self.save_best_model('accuracy')
